...
def newinfo(uid, region="br"):
    try:
        url = f"https://freefirefwx-beta.squareweb.app/api/info_player?uid={uid}&region={region}&clothes=true"
...
def send_likes(uid):
    try:
        url = f"https://world-ecletix.onrender.com/api/likes?id={uid}"
        response = requests.get(url)

        if response.status_code == 200:
            api_data = response.json()
            
            if not api_data.get("status") or not api_data.get("resultado"):
                # Caso a API retorne status false ou resultado vazio
                return {
                    "status": "failed",
                    "message": (
                        f"[C][B][FF0000]________________________\n"
                        f" ❌ Não foi possível enviar likes!\n"
                        f" Verifique se o ID está correto\n"
                        f"________________________"
                    )
                }
            
            resultado = api_data["resultado"]
            likes_ganhos = resultado.get("likesGanhos", 0)

            if likes_ganhos == 0:
                # Caso limite diário atingido (vermelho)
                return {
                    "status": "failed",
                    "message": (
                        f"[C][B][FF0000]________________________\n"
                        f" ❌ Limite diário de likes atingido!\n"
                        f" Tente novamente em 24 horas\n"
                        f"________________________"
                    )
                }
            else:
                ...
                return {
                    ...
                    "message": (
                        ...
                        f" ✅ {likes_ganhos} likes enviados com sucesso!\n"
                        f" Nome: {resultado.get('nick')}\n"
                        f" Likes antes: {resultado.get('likesAntes')}\n"
                        f" Likes depois: {resultado.get('likesDepois')}\n"
                        f" Nível: {resultado.get('nivel')}\n"
                        f" Avatar: {resultado.get('avatar')}\n"
                        f"________________________"
                    )
                }
        else:
...
[FFFFFF]Para suporte técnico, envie uma mensagem ao desenvolvedor 📩  
[FFD700]INSTA : [FFFF00]@comedor_di_primas
[1E90FF]╚══════════════════════════╝""", idinv
                        )
                )
                senthi = False
...
[b][i][A5E2CFٍ]Instagram: @comedor_di_primas[/b]
...
[C][B][FF6347] Developer    luanzin """, uid
    ...
    try:
        ...
        if len(command_split) <= 1 or not command_split[1].strip():
            ...
            clients.send(self.GenResponsMsg("[C][B][FF0000] Por favor insira um [00FF00]ID válido!", sender_id))
        else:
            # Extrai UID só de números
            uids = re.findall(r"\b\d{5,15}\b", command_split[1])
            uid = uids[0] if uids else ""

            if not uid:
                print("❌ UID inválido.")
                clients.send(self.GenResponsMsg("[C][B][FF0000] ID do jogador inválido!", sender_id))
            else:
                print(f"✅ UID extraído: {uid}")

                try:
                    info_response = newinfo(uid)  # usa a função já corrigida da nova API
                    print(f"✅ API Response: {info_response}")  
                except Exception as e:
                    print(f"❌ Erro API: {e}")
                    clients.send(self.GenResponsMsg("[C][B][FF0000] Erro no servidor, tente novamente!", sender_id))
                    return
                ...
                if 'info' not in info_response or info_response['status'] != "ok":
                    ...
                    print("✅ Dados recebidos, formatando...")  
                    ...
                    # Info do clã
                    if clan_info == "false":
                        clan_info_text = "\n[B][FF0000]• Jogador não está em nenhum clã\n"
                    else:
                        clan_info_text = (
                            f"[B][FFA500]• Clã: [FFFFFF]{clan_info.get('clanName', 'N/A')}\n"
                            f"[B][FFA500]• ID: [FFFFFF]{clan_info.get('clanId', 'N/A')}\n"
                            f"[B][FFA500]• Nível: [FFFFFF]{clan_info.get('clanLevel', 0)}\n"
                            f"[B][FFA500]• Membros: [FFFFFF]{clan_info.get('memberNum', 0)}/{clan_info.get('capacity', 0)}\n"
                        )
                    ...
                    bio = infoo.get('socialInfo', {}).get('signature', 'Sem bio').replace("|", " ")

                    print(f"✅ Player Info: {name}, Lvl {level}, {region}")

                    # Mensagem final
                    message_info = (
                        f"[C][B][00FF00]«—————— Player Info ——————»\n"
                        f"[B][FFA500]• Nome: [FFFFFF]{name}\n"
                        f"[B][FFA500]• Level: [FFFFFF]{level}\n"
                        f"[B][FFA500]• Região: [FFFFFF]{region}\n"
                    ...
                    try:
                        ...
                        clients.send(self.GenResponsMsg("[C][B][FF0000] Falha ao enviar mensagem!", sender_id))

    except Exception as e:
        print(f"❌ Erro inesperado em /info: {e}")
        clients.send(self.GenResponsMsg("[C][B][FF0000] Ocorreu um erro inesperado!", sender_id))            
	                    
	                    
	                    
	           if "1200" in data.hex()[0:4] and b"/likes" in data:
    try:
        print("✅ /likes command detected.")
        command_split = re.split("/likes", str(data))

        # Pega informações do sender
        json_result = get_available_room(data.hex()[10:])
        parsed_data = json.loads(json_result)
        sender_id = parsed_data["5"]["data"]["1"]["data"]
        sender_name = parsed_data['5']['data']['9']['data']['1']['data']
        ...
        # Verifica se mandaram ID
        if len(command_split) <= 1 or not command_split[1].strip():
            print("❌ Nenhum ID fornecido, enviando aviso.")
            clients.send(self.GenResponsMsg("[C][B][FF0000] Por favor insira um ID válido de jogador!", sender_id))
        else:
            # Extrai somente números (UID)
            uids = re.findall(r"\b\d{5,15}\b", command_split[1])
            ...
            if not player_id:
                ...
                print(f"✅ Player ID para likes: {player_id}")

                # Resposta inicial (processando)
                clients.send(self.GenResponsMsg(f"{generate_random_color()}🔄 Processando pedido de likes...", sender_id))

                try:
                    likes_response = send_likes(player_id)
                    status = likes_response.get("status", "error")
                    message = likes_response.get("message", "[C][B][FF0000] Erro ao enviar likes!")

                    print(f"✅ Likes Response: {likes_response}")
                    clients.send(self.GenResponsMsg(message, sender_id))
                except Exception as e:
                    print(f"❌ Erro no envio de likes: {e}")
                    clients.send(self.GenResponsMsg("[C][B][FF0000] Falha ao processar o pedido de likes!", sender_id))

    except Exception as e:
        print(f"❌ Erro inesperado em /likes: {e}")
        clients.send(self.GenResponsMsg("[C][B][FF0000] Ocorreu um erro inesperado!", sender_id))
	            ...
	            if "1200" in data.hex()[0:4] and b"/help" in data:
	                ...
	                clients.send(
	                    self.GenResponsMsg(
		                        f"""[B][C][FFFF00] BEM-VINDO AO [ffffff]MTF BOT  !! 

[C][B][00FF00]/🤔likes [id] -> [C][B][FFFFFF]mande 100 likes
		
[FF0000] /🤔info [id] -> [C][B][FFFFFF]info da conta
	
[00FF00َ] /🤔status[id] ->  [C][B][FFFFFF]status do jogador
...
[00FF00َ] /🤔ai [word] -> [C][B][FFFFFF]ia dentro do ff [ BHARAT AI ]
...
[00FF00َ] /🤔admin -> [C][B][FFFFFF]Saiba sobre os administradores do bot
	 
/🤔inv [ID] -> [C][B][FFFFFF]Convidar qualquer jogador aleatório
-------------------------------
...
[00FF00] /🤔3 -> /🤔4 -> /🤔5 -> /🤔6""", uid
	                ...
	                clients.send(
		                    self.GenResponsMsg(
		                        f"""		-------------------------------
[00FF00َ] /🤔lag (código da equipe) -> [C][B][FFFFFF]Deixar qualquer equipe lenta

[FF0000] /🤔lag (código da equipe) 2 -> [C][B][FFFFFF]Deixar qualquer equipe lenta

[00FF00] /🤔attack (código da equipe) -> [C][B][FFFFFF]Entrar e atacar qualquer equipe

[40E0D0] /🤔start (código da equipe) -> [C][B][FFFFFF]Forçar qualquer equipe a iniciar o jogo
		                       """, uid
	                    )
	                )
...
if __name__ == "__main__":
    try:
        client_thread = FF_CLIENT(id="4148514165", password="C8DD3A6F4A8272966D44F1845AD599769D9B8E47B7F0D204F4ED1BEE86A762CE")
        client_thread.start()
    except Exception as e:
