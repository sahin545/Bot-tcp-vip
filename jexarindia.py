...
[C][B][FF6347] Developer    luanzin """, uid
    ...
    try:
        ...
        if len(command_split) <= 1 or not command_split[1].strip():
            clients.send(self.GenResponsMsg("[C][B][FF0000] Por favor insira um [00FF00َ]ID válido de jogador!", sender_id))
        else:
            # Extrai UID
            ...
            if not uid:
                ...
                if info_response['status'] != "ok":
                    clients.send(self.GenResponsMsg("[C][B][FF0000] ID errado ou erro na API, verifique novamente", sender_id))
                else:
                    infoo = info_response['info']
                    basic_info = infoo['basic_info']
                    clan_info = infoo.get('clan_info', "false")
                    ...
                    # Clan text
                    if clan_info == "false":
                        clan_info_text = "\nJogador não está em nenhum Clã\n"
                    else:
                        clan_info_text = (
                            f" Clã Info :\n"
                            f"ID: {fix_num(clan_info['clanId'])}\n"
                            f"Nome: {clan_info.get('clanName', 'N/A')}\n"
                            f"Nível: {clan_info.get('clanLevel', 0)}\n"
                            f"Membros: {clan_info.get('memberNum', 0)}\n"
                        )
                    ...
                    region = basic_info['region']
                    bio = data.get('socialInfo', {}).get('signature', "Sem bio").replace("|", " ")

                    message_info = (
                        f"[C][B][00FF00]«—————— Player Info ——————»\n"
                        f"[B][FFA500]• Nome: [FFFFFF]{name}\n"
                        f"[B][FFA500]• Nível: [FFFFFF]{level}\n"
                        f"[B][FFA500]• Servidor: [FFFFFF]{region}\n"
                        f"[B][FFA500]• Likes: [FFFFFF]{fix_num(likes)}\n"
                        f"[B][FFA500]• Bio: [FFFFFF]{bio}\n"
                        f"{clan_info_text}\n"
                    )
